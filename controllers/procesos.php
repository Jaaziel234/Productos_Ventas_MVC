<?php
    ...
    function CargarIMG($stmdir, $imgsize, $newname, $path)
    {
        if (!is_dir($path)) {
            mkdir($path, 0777, true);
        }
        if (is_uploaded_file($stmdir)) {
            copy($stmdir, $path.$newname);
            return $subido= true;
        } 
        else {
            return $subido= false;
    ...
    function CargarIMGEDIT($stmdir, $imgsize, $newname, $path, $old)
    {
        if (!is_dir($path)) {
            mkdir($path, 0777, true);
        }
